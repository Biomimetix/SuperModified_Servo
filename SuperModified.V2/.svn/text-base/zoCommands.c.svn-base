...
/*13*/	void zoCommandConfigureDigitalIO(ZO_PROTOCOL_PACKET *packet)
{
	...
	enterCritical();
	//set digital IO 1 direction -> DD7
	DDRD = (DDRD & (~0x80)) | ( (dir<<7) & 0x80 );
	//set digital IO 4 direction -> DD6
	DDRD = (DDRD & (~0x40)) | ( (dir<<3) & 0x40 );
	//set digital IO 2,3 direction -> DDRB0,1 respectively
	DDRB = (DDRB & (~0x03)) | ( (dir>>1) & 0x03 );
...
/*14*/	void zoCommandSetDigitalOut(ZO_PROTOCOL_PACKET *packet)
{
	...
	//set digital out 1 -> PD7
	PORTD = (PORTD & (~0x80)) | ( (out<<7) & 0x80 );
	//set digital out 4 -> PD6
	PORTD = (PORTD & (~0x40)) | ( (out<<3) & 0x40 );
	//set digital IO 2,3 -> portB2,3 respectively
	PORTB = (PORTB & (~0x03)) | ( (out>>1) & 0x03 );
	exitCritical();
}
...
/*6D*/	void zoCommandGetDigitalIn(ZO_PROTOCOL_PACKET *packet)
{
	...
	enterCritical();
	//Get digital in 1 from PD7
	in = (PIND & 0x80) >> 7;
	//Get digital in 4 from PD6
	in |= (PIND & 0x40) >>3 ;
	in |= (PINB & 0x03) << 1;
	exitCritical();
