...
//RS485 boot enable: 1->enabled, anything else disabled
#define RS485_BOOT 	1
...
void putch(char ch)
{
	...
	while (!(UCSR0A & _BV(UDRE0)));
#if RS485_BOOT == 1
	PORTD |= _BV(PD2);					//capture bus
#endif
	UDR0 = ch;
#if RS485_BOOT == 1
	while(!(UCSR0A & _BV(TXC0)));		//wait until tx complete
	UCSR0A |= _BV(TXC0);				//force txc flag reset
	PORTD &= ~_BV(PD2);				//release bus
#endif
#else
